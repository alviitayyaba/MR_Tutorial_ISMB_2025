...
## Part 1: Performing the Core MR Analysis
We will start by performing the primary MR analysis using several robust methods: Inverse Variance Weighted (IVW), MR Egger regression, and Weighted Median.

```{r  message=FALSE, warning=FALSE}
# Perform MR analysis using specified methods
mr_results <- mr(ldl_cad_fil, method_list = c("mr_egger_regression", "mr_ivw", "mr_weighted_median"))
...
Based on the mr_results table, what are the causal effect estimates (b) and p-values (pval) for LDL on CAD for each of the three MR methods (IVW, MR Egger, Weighted Median)? Which methods suggest a statistically significant causal effect at 
alpha=0.05?
...
Interpret the Q-statistic and its p-value for both the MR Egger and IVW methods. What do these results suggest about the presence of heterogeneity in our analysis?
...
## Part 4: Investigating Pleiotropy with the Funnel Plot
...
## Part 5: Quantifying Pleiotropy with MR Egger Intercept
...
Given the heterogeneity and pleiotropy test results, which MR method's estimate (IVW, MR Egger, or Weighted Median) do you think is most reliable in this specific scenario, and why?

## Part 6: Leave-One-Out Analysis
