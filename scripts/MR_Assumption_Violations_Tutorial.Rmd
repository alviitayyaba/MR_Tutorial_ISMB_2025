...
This tutorial demonstrates how Mendelian Randomization (MR) results can be biased when its core assumptions are violated. Each section introduces a potential violation and how to detect or mitigate it using simulated data. Use this as a step-by-step hands-on session.
...
Explain LD clumping and simulate it by keeping only SNPs on different chromosomes.
...
## Part 2: No Pleiotropy (MR Assumptions Hold)
```
```{r part2-LD-clumping}
...
## Part 4: Harmonization and Palindromic SNPs
...
## Part 5: Invalid Instruments & Weak Instrument Bias
...
exposure_gwas_weak$beta.exposure[1:3] <- 0.001
exposure_gwas_weak$pval.exposure[1:3] <- 0.8

# Harmonize
harm_weak <- harmonise_data(exposure_gwas_weak, outcome_gwas)

# MR with weak instruments
mr_results_weak <- mr(harm_weak)
mrobust <- mr_heterogeneity(harm_weak)
ggplot(mr_results_weak, aes(x = b, y = method)) +
  geom_point(size = 3) +
  geom_errorbar(aes(xmin = b - se, xmax = b + se), width = 0.2) +
  labs(x = "MR Method", y = "Causal Estimate", title = "MR estimates with weak instruments") +
  theme_bw()
```
...
outcome_gwas_pleio <- outcome_gwas
outcome_gwas_pleio$beta.outcome[4:6] <- outcome_gwas_pleio$beta.outcome[4:6] + 0.2

harm_pleio <- harmonise_data(exposure_gwas_clumped, outcome_gwas_pleio)

mr_egger <- mr(harm_pleio, method_list = c("mr_ivw","mr_egger_regression"))
pleio_test <- mr_pleiotropy_test(harm_pleio)
print(pleio_test)
mr_scatter_plot(mr_egger, harm_pleio)
...
## Part 7: Addressing Pleiotropy via Confounder Filtering
...
ggplot(mr_clean, aes(x = b, y = method)) +
  geom_point(size = 3) +
  geom_errorbar(aes(xmin = b - se, xmax = b + se), width = 0.2) +
  labs(x = "MR Method", y = "Causal Estimate",
       title = "MR estimates after filtering confounding SNPs") +
  theme_bw()
```
